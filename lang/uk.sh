...
MI_PANIC_TRIPPED="!! Аварійний гальм уже спрацьовував — сервіс вимикав сам себе:"
MI_PANIC_TRACE="   Слід: %s (прибери вручну, коли розберешся з причиною)\n"
...
MI_PANIC_HEADER="== Аварійний гальм (необов'язковий) =="
MI_PANIC_WHAT="Якщо CPU тримається на T_PANIC задану кількість замірів поспіль — тобто
фан уже на максимумі й не витягує — демон:
  1. вимикає власний unit (щоб не вийшов цикл ребутів)
  2. жене продув, поки пакет не охолоне до PANIC_COOL_TO
  3. віддає керування назад і спостерігає PANIC_WATCH_SEC
  4. ребутає ЛИШЕ якщо температура знову рвонула до PANIC_WATCH_TRIP;
     інакше — помилування: unit вмикається назад, робота триває"
...
навантаженням при тротлінгу CPU на 100°C. Замалий поріг означає ребут
сервера посеред звичайної збірки."
MI_PANIC_ASK="Увімкнути аварійний гальм?"
MI_Q_TPANIC="T_PANIC — температура (°C), яка запускає гальм"
MI_Q_PSAMPLES="PANIC_SAMPLES — скільки замірів поспіль має протриматись"
MI_PANIC_COOL_WHY="   Перед ребутом демон тримає фан у продуві, поки пакет не охолоне:
   під час POST вентилятором ніхто не керує, і гарячий проц пройшов би
   перезавантаження без обдуву."
MI_Q_COOLTO="PANIC_COOL_TO — до якої температури (°C) остудити перед ребутом"
MI_Q_COOLTMO="PANIC_COOL_TIMEOUT — скільки максимум чекати остигання, сек"
MI_PANIC_WATCH_WHY="   Після остигання демон спостерігає: якщо машина лишається холодною,
   вона впоралась, і ребутати її було б чистою шкодою."
MI_Q_WATCHSEC="PANIC_WATCH_SEC — скільки спостерігати після остигання, сек"
MI_Q_WATCHTRIP="PANIC_WATCH_TRIP — до якої (°C) має рвонути назад, щоб таки ребутнути"
MI_PANIC_LOOP_WHY="   Без обмеження це може зациклитись: нагрів -> остигання -> помилування
   -> нагрів, і гальм ніколи не спрацює. 0 = ребутати завжди одразу."
MI_Q_REPRIEVES="PANIC_REPRIEVES — скільки помилувань дозволено"
...
MI_SUM_PANIC="          гальм при %s°C, витриманих %sс,
          остигання до %s°C (не довше %sс), спостереження %sс,
          ребут лише при поверненні до %s°C; помилувань: %s\n"
MI_SUM_PANIC_OFF="          аварійний гальм вимкнено"
...
MI_ALARM_HEADER="== Пікання на старті, якщо демон не піднявся (необов'язково) =="
MI_ALARM_WHAT="Усі аварійні шляхи тут закінчуються поверненням до BIOS auto, а гальм
ще й вимикає власний unit перед ребутом — тож машина, яка не витягнула
охолодження, вертається у стоковий режим тихо й мовчки. Окремий
маленький сервіс через %sс після старту дивиться, чи працює
optiplex-fan, і якщо ні — пікає внутрішнім спікером:
  два коротких піки  unit увімкнений, але не стартонув
  три довгих піки    unit вимкнений — саме це лишає по собі гальм
На тому й виходить. Поки демон працює, ніщо не пікає, і ніхто не нудить
після того. Це окремий unit, тож гальм не може його заглушити."
MI_ALARM_ASK="Поставити пікання на старті?"
...
MU_PANIC_KEPT="Слід аварійних спрацювань лишено: /var/lib/optiplex-fan/panic"
